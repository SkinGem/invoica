...
## 29. npm Publish Pipeline: OIDC vs Token Auth Battle (2026-03-02 → 2026-03-08)

**Root cause:** 8 commits trying different npm auth strategies — OIDC Trusted Publishers, Sigstore provenance, registry-url settings, npm ci vs npm install — before reaching a stable solution.

**Impact:** `@invoica/sdk` and `@invoica/mcp` publish pipeline was broken or unreliable for ~1 week.

**Fix:** Granular Access Token stored as `NPM_TOKEN1` secret (not `NPM_TOKEN`). `npm install` instead of `npm ci` for the sdk package. Sigstore provenance retained.

**Rules going forward:**
- Use Granular Access Token for npm publish — not OIDC trusted publishers
- Secret name is `NPM_TOKEN1` — never rename without updating all workflows
- Use `npm install`, not `npm ci`, in the SDK workflow (peer deps conflict with ci's strict mode)
- Daily npm release status report now generated by CEO agent to catch failures early

---

## 30. MCP Server Distribution via npx (2026-03-08)

**Root cause / context:** MCP package (`@invoica/mcp`) needed separate npm package scaffold — different from main backend. Sprint runner not suitable for this; required direct session.

**Fix:** Shipped `@invoica/mcp` as standalone npm package. `npx -y @invoica/mcp` connects to Invoica API. PM2 process `mcp-invoica` runs the server. Tools: `create_invoice`, `list_invoices`, `check_settlement`.

**Rules going forward:**
- New npm package scaffolding (new `package.json`) = owner-direct session, not sprint runner
- Keep `mcp-package/` directory separate from main `package.json` — different deps, different CI
- MCP tools must match the live API contract exactly — not aspirational features

---

## 31. Week-76 Bug Pattern: Cross-Layer Integration Tests Missing (2026-03-09)

**Root cause:** All 5 Week-76 bugs were at service boundary mismatches — route called wrong service, service called non-existent Supabase table, stub returned hardcoded responses. None caught by existing tests because there were zero integration tests.

| Bug | Root cause |
|-----|------------|
| FIX-001: rotate/delete KEY_NOT_FOUND | Route called `ApiKeyRotationService` querying a non-existent Supabase table; correct functions already existed in `services/api-keys.ts` |
| FIX-002: invoice numbers = Unix timestamps | Test data inserted `Date.now()` as `invoiceNumber`; `MAX()+1` propagated it to all subsequent invoices |
| FIX-003: API keys lost on PM2 restart | `createApiKey()` wrote to in-memory store only — no Supabase persistence |
| FIX-004: wrong auth error message | Error said `Bearer` only; primary header is `X-API-Key` |
| FIX-005: webhooks were a complete stub | POST/GET/DELETE never touched DB; always returned fake success |

**Rules going forward:**
- Every new API route touching >1 service layer requires at least 1 integration test (route → service → DB roundtrip)
- Test data must use clearly flagged values (`TEST_` prefix or test DB) — never `Date.now()` as a business-domain ID
- Sprint zero-test rule: no sprint closes without a test coverage check

---

## 32. CEO Auto-Sprint Path Hallucination (2026-03-04 → ongoing)

**Root cause:** CEO generates sprint tasks targeting `src/lib/x402.ts`, `src/config/chains.ts`, `src/components/ui/`, `src/styles/globals.css` — old Next.js project paths that no longer exist. Current monorepo uses `backend/src/`, `frontend/src/`, `agents/`, `scripts/`.

**Impact:** Tasks skipped or rejected with "file not found". Wasted sprint runner cycles.

**Fix:** Updated CEO sprint context with correct directory map. `CLAUDE.md` file structure section is the canonical reference.

**Rules going forward:**
- CEO sprint context must always be refreshed with `ls -R backend/src/ frontend/src/` at generation time
- Any task targeting a path containing `src/lib/`, `src/config/`, `src/styles/` with no `backend/` or `frontend/` prefix = auto-rejected as path hallucination
- File structure section in `CLAUDE.md` is the single source of truth for valid paths

---

## 33. Agent File Truncation and Markdown Fence Bug (2026-03-04 → ongoing)

**Root cause:** MiniMax M2.5 and Claude Sonnet agents frequently truncate large file outputs (>150 lines) or wrap TypeScript in markdown fences (` ```typescript `). Both patterns cause quality gate hard rejections.

**Impact:** `ORCH-001` blocked for 4+ sprints. Every large-file task has elevated rejection rate (~60% when file >150 lines).

**Rules going forward:**
- Every agent prompt must include: "Output raw TypeScript only. Do NOT wrap in markdown code fences. Do NOT truncate."
- Tasks touching files >150 lines must be decomposed into sub-files before assigning to sprint agents
- Quality gate rejects markdown fences (correct behavior) — do not loosen this rule

---

## 34. orchestrator.ts Cascade Block — ORCH-001 (2026-03-09 → 2026-03-10)

**Root cause:** `orchestrator.ts` is the largest file in the codebase (~250+ lines). Auto-decomposition task (`ORCH-001`) kept failing: wrong rejection threshold (2 instead of 5), output truncated, markdown fence wrapping. CEO flagged 0% task completion across 3 sprints because `current.json` symlink pointed to stale `week-21.json`.

**Compounding factor:** `current.json` symlink was pointing to `week-21.json` instead of the latest sprint, causing the CEO review to see artificially 0% completion and trigger an "EXECUTION CRISIS" sprint.

**Fix:** 
1. Merged `origin/orchestrator.ts` to unblock other sprints
2. Updated `current.json` symlink → `week-76.json`
3. ORCH-001 backlogged for owner-direct refactor session

**Rules going forward:**
- `current.json` symlink must always point to the most recently completed named sprint — verify after every manual sprint
- Files >200 lines must be refactored in owner-direct sessions, not via sprint runner
- `orchestrator.ts` is permanently flagged as "manual-only" in sprint context

---

## 35. Invoice Number Sequence Corruption (2026-03-09)

**Root cause:** Unix timestamp `1772554801` was stored as `invoiceNumber` during a test run. Invoice generation uses `MAX(invoiceNumber) + 1`, so every subsequent invoice received a 10-digit number (1772554802, 1772554803, ...).

**Fix:** `scripts/fix-invoice-sequence.sql` — `DELETE FROM "Invoice" WHERE "invoiceNumber" > 1000000000`. 3 test rows removed, 5 real invoices remain. Sequence reset.

**Rules going forward:**
- Never use `Date.now()`, Unix timestamps, or random large integers as business-domain sequence IDs during testing
- Test invoice creation must use a dedicated test user account that is excluded from sequence calculations
- Add DB constraint: `invoiceNumber < 100000` to prevent this class of corruption

---

## 36. CEO Bot Token Waste: Context Rebuilt 25× Per Message (2026-03-09)

**Root cause:** `ceoBot.ts` bypasses `agent.yaml` model router entirely — hardcodes `claude-sonnet-4-5` via direct Anthropic API calls. `buildLiveContext()` (reads health.json, PM2 jlist, memory files, sprint files) was called on every iteration of a `while (iterations < 25)` agentic loop — up to 25 filesystem reads per user message.

**Fix:**
- Model router: `STRATEGIC_RE` keyword match → Sonnet; everything else → Haiku
- `buildLiveContext()` moved outside the loop — called once per user message
- Loop cap: 25 → 8 iterations
- `max_tokens`: 4096 (Sonnet) / 1024 (Haiku)

**Rules going forward:**
- `ceoBot.ts` is a standalone agent — `agent.yaml` config does NOT apply to it
- Any context-building function (filesystem reads, DB queries) must be outside agentic loops
- Model routing: Haiku for operational queries (<7 history messages + no strategic keywords), Sonnet for strategy/planning

---

## 37. X-Admin Never Posting: CMO Updates Slot Was 100% Roadmap Content (2026-03-09)

**Root cause:** CMO weekly plan used "Building:", "Working on:", "Debugging:", "Hardening:" prefixes in every updates slot — all roadmap reveals. CEO review gate (correctly) rejected 100% of updates slot content every 30-minute cycle. Other slots (educational, vision) were passing fine. Total posts from Feb 26 launch to Mar 2: 0 (also blocked by EACCES permission crash on `reports/invoica-x-admin/`).

**Fix:**
1. Rewrote all 7 days of updates slots with Week-76 shipped-fix content (past tense, specific technical details)
2. Added `FORCE_SLOT=updates` env var to `run-x-admin.ts` for manual override when a window is missed
3. Hardened CMO prompt in 3 layers: Manus prompt, Claude fallback prompt, CEO review gate — specific forbidden words and product names listed
4. Injected `docs/x-posting-guidelines.md` (X algorithm signal weights, penalty signals, format hierarchy) into CMO weekly plan generation

**Rules going forward:**
- Updates slot = shipped features only. Past tense verbs: "Fixed:", "Shipped:", "Patched:"
- Forbidden in updates: "Building:", "Working on:", "Planned:", "Coming soon:", product names from roadmap (Gas Backstop, SOL Incinerator, Reputation API, Agent Marketplace)
- CMO generates weekly plan on Sunday 06:00 UTC — review it Monday morning and patch any roadmap leaks before the first updates window fires at 13:00 UTC
- FORCE_SLOT usage: `FORCE_SLOT=<slot> TS_NODE_TRANSPILE_ONLY=true TS_NODE_PROJECT=./tsconfig.json node -r ts-node/register scripts/run-x-admin.ts`

---

## 38. Watchdog Alert Spam: No Deduplication (2026-03-10)

**Root cause:** `pm2-process-watchdog.ts` sent a Telegram alert on every 5-minute cron tick when any critical process was down. No throttling. One downed process = 12+ Telegram messages per hour.

**Fix:** Added 30-minute per-process throttle using `logs/watchdog-last-alert.json` state file. Per-process granularity: `backend` throttle does not block `ceo-ai-bot` alert.

**Rules going forward:**
- All alerting systems must include deduplication — alert once per incident, not once per check interval
- Throttle state file location: `logs/watchdog-last-alert.json`
- Reset: delete the state file to force immediate alert on next watchdog run

---

## 39. openclaw-gateway Port Conflict: Orphan Process (recurring)

**Root cause:** When PM2 aggressively restarts `openclaw-gateway`, sometimes the previous process remains as an orphan holding port 18789. The gateway wrapper detects the port conflict, exits 0 (not an error exit), so PM2 marks status=`online` despite the gateway not actually serving. PM2 crash loop at ~5 restarts/minute. Watchdog sees `online` and sends no alert. `lsof` was not installed, making orphan detection impossible.

**Fix:** `fuser -k 18789/tcp` kills the orphan. `apt install lsof` for diagnostics. `pm2 restart openclaw-gateway` after port is clear.

**Recurrence prevention:**
- Add port-check to watchdog: `nc -z localhost 18789` — if port not responding, treat as down regardless of PM2 status
- `lsof` is now installed on the Hetzner VPS
- Recovery command: `fuser -k 18789/tcp && sleep 2 && pm2 restart openclaw-gateway`
