...
export default function ApiKeysPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {/* Error state */}
      {error && (
        <div className="flex items-center gap-2 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/30 rounded-xl px-4 py-3 mb-6">
          <svg className="w-4 h-4 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
          </svg>
          <p className="text-sm text-red-600 dark:text-red-400">{error}</p>
          <button onClick={fetchKeys} className="ml-auto text-xs text-red-500 dark:text-red-400 underline">Retry</button>
        </div>
      )}

      {/* Loading skeleton */}
      {loading && !error && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm animate-pulse">
          ...
          {[1, 2, 3].map((i) => (
            <div key={i} className="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center gap-4">
              ...
              <div className="h-4 bg-gray-200 dark:bg-gray-700 rounded w-40 font-mono" />
      ...
      {!loading && !error && keys.length === 0 && (
        <div className="bg-white dark:bg-gray-900 border border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-12 text-center">
          ...
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-1">No API keys yet</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Create your first key to start using the Invoica API.</p>
          <button
            onClick={() => setShowCreateModal(true)}
      ...
      {!loading && !error && activeKeys.length > 0 && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm mb-4">
          <div className="px-6 py-3.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50">
          ...
          <div className="divide-y divide-gray-100">
            {activeKeys.map((key) => (
              <div key={key.id} className="px-6 py-4 flex items-center gap-4 group hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <code className="text-xs text-gray-500 dark:text-gray-400 font-mono">
                    sk_<span className="text-gray-400 dark:text-gray-500">{key.keyPrefix}</span>••••••••••••••••
                  </code>
                </div>
                <div className="hidden sm:flex items-center gap-6 text-xs text-gray-500 dark:text-gray-400 shrink-0">
                  <div className="text-right">
                    <p className="text-gray-400 dark:text-gray-500 uppercase tracking-wide text-[10px] font-medium">Created</p>
                    <p className="text-gray-600 dark:text-gray-300 font-medium">{formatDate(key.createdAt)}</p>
                  </div>
                  {key.lastUsedAt && (
                    <div className="text-right">
                      <p className="text-gray-400 dark:text-gray-500 uppercase tracking-wide text-[10px] font-medium">Last used</p>
                      <p className="text-gray-600 dark:text-gray-300 font-medium">{formatDate(key.lastUsedAt)}</p>
                    </div>
                  )}
                  {key.expiresAt && (
                    <div className="text-right">
                      <p className="text-gray-400 dark:text-gray-500 uppercase tracking-wide text-[10px] font-medium">Expires</p>
                      <p className="text-amber-600 dark:text-amber-400 font-medium">{formatDate(key.expiresAt)}</p>
                    </div>
                  )}
                </div>
                <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 dark:bg-green-950/30 text-green-700 dark:text-green-400 shrink-0">
                  Active
                </span>
                <button
      ...
      {!loading && !error && revokedKeys.length > 0 && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm opacity-70">
          <div className="px-6 py-3.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50">
          ...
          <div className="divide-y divide-gray-100">
            {revokedKeys.map((key) => (
              <div key={key.id} className="px-6 py-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-500 truncate line-through">{key.name}</p>
                  <code className="text-xs text-gray-400 dark:text-gray-600 font-mono">
                    sk_<span>{key.keyPrefix}</span>••••••••••••••••
                  </code>
                </div>
                <div className="hidden sm:flex items-center gap-6 text-xs text-gray-400 dark:text-gray-600 shrink-0">
                  <div className="text-right">
                    <p className="text-gray-300 dark:text-gray-600 uppercase tracking-wide text-[10px] font-medium">Revoked</p>
                    <p className="font-medium">{formatDate(key.updatedAt)}</p>
                  </div>
                </div>
                <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 dark:bg-red-950/30 text-red-500 dark:text-red-400 shrink-0">
                  Revoked
                </span>
              </div>
            ))}
