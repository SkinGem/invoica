...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    active: 'bg-green-100 dark:bg-green-950/30 text-green-800 dark:text-green-400',
    trialing: 'bg-blue-100 dark:bg-blue-950/30 text-blue-800 dark:text-blue-400',
    past_due: 'bg-yellow-100 dark:bg-yellow-950/30 text-yellow-800 dark:text-yellow-400',
    canceled: 'bg-red-100 dark:bg-red-950/30 text-red-800 dark:text-red-400',
...
export default function BillingPage() {
  ...
  if (loading) {
    return (
      <div className="max-w-3xl mx-auto p-6">
        <h1 className="text-3xl font-bold mb-6 dark:text-white">Billing</h1>
  ...
  if (error && !billing) {
    return (
      <div className="max-w-3xl mx-auto p-6">
        <h1 className="text-3xl font-bold mb-6 dark:text-white">Billing</h1>
        <div className="border border-red-200 dark:border-red-800/30 bg-red-50 dark:bg-red-950/30 rounded-lg p-6 text-center">
          <p className="text-red-600 dark:text-red-400 mb-3">{error}</p>
          <button onClick={loadBilling} className="text-sky-600 dark:text-sky-400 hover:underline text-sm">
            Try again
          </button>
        </div>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/30 rounded-lg text-red-600 dark:text-red-400 text-sm">
          {error}
        </div>
      )}
      ...
      <section className="mb-8">
        ...
        <div className="border dark:border-gray-800 rounded-lg p-6 dark:bg-gray-900">
          <div className="flex justify-between items-start mb-4">
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h3 className="text-xl font-bold capitalize dark:text-white">{plan} Plan</h3>
          ...
          <div className="flex gap-3">
            ...
            {(isPro || plan === 'enterprise') && billing?.stripe_customer_id && (
              <button
                ...
                disabled={actionLoading}
                className="px-5 py-2.5 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-800 transition-all disabled:opacity-50"
              >
      ...
      {/* Usage */}
      <section className="mb-8">
        <h2 className="text-xl font-semibold mb-4 dark:text-white">Usage This Month</h2>
      ...
      {isFree && (
        <section>
          <h2 className="text-xl font-semibold mb-4 dark:text-white">Why Upgrade?</h2>
          <div className="border dark:border-gray-800 rounded-lg p-6 bg-gradient-to-br from-sky-50 to-blue-50 dark:from-sky-950/20 dark:to-blue-950/20">
            <ul className="space-y-3">
              {[
                ...
                <li key={feature} className="flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300">
                  <svg className="w-5 h-5 text-sky-600 dark:text-sky-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
