...
<div class="footer">Powered by Invoica &mdash; x402 Protocol &bull; invoica.ai</div>
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Invoices</h1>
      {invoices.length === 0 ? (
        <div className="text-center py-20 text-slate-500 dark:text-gray-400">No invoices found.</div>
      ) : (
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-slate-200 dark:border-gray-700 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-gray-800 border-b border-slate-200 dark:border-gray-700">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-slate-600 dark:text-gray-300">Invoice</th>
                <th className="text-left px-4 py-3 font-medium text-slate-600 dark:text-gray-300">Seller</th>
                <th className="text-left px-4 py-3 font-medium text-slate-600 dark:text-gray-300">Amount</th>
                <th className="text-left px-4 py-3 font-medium text-slate-600 dark:text-gray-300">Status</th>
                <th className="text-left px-4 py-3 font-medium text-slate-600 dark:text-gray-300">Created</th>
                <th className="text-right px-4 py-3 font-medium text-slate-600 dark:text-gray-300">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-gray-800">
              {invoices.map((invoice) => (
                <tr key={invoice.id} className="hover:bg-slate-50 dark:hover:bg-gray-800">
                  <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{invoice.invoiceNumber}</td>
                  <td className="px-4 py-3 text-slate-700 dark:text-gray-300">{invoice.sellerName || '-'}</td>
                  <td className="px-4 py-3 text-slate-700 dark:text-gray-300">
                    {invoice.currency} {(invoice.total || invoice.amount).toFixed(2)}
                    {(invoice.taxAmount || 0) > 0 && (
                      <span className="text-xs text-slate-400 dark:text-gray-500 ml-1">(incl. tax)</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                      invoice.status === 'completed' || invoice.status === 'settled'
                        ...
                        : invoice.status === 'pending' ? 'bg-yellow-100 dark:bg-yellow-950/30 text-yellow-700 dark:text-yellow-400'
                        : invoice.status === 'overdue' ? 'bg-red-100 dark:bg-red-950/30 text-red-700 dark:text-red-400'
                        : 'bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-gray-400'
                    }`}>
                      {invoice.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-slate-500 dark:text-gray-400">{new Date(invoice.createdAt).toLocaleDateString()}</td>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-8">
              ...
              <div className="flex justify-between items-start mb-10">
                <div>
                  <div className="text-2xl font-bold text-[#0A2540] dark:text-white">{previewInvoice.sellerName || 'Seller'}</div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-bold text-[#0A2540] dark:text-white">INVOICE</div>
                  <div className="text-sm text-slate-500 dark:text-gray-400 mt-1">{previewInvoice.invoiceNumber}</div>
              ...
              <div className="grid grid-cols-2 gap-8 mb-10">
                <div>
                  <div className="text-xs uppercase tracking-wider text-slate-400 dark:text-gray-500 mb-2">Bill To</div>
              ...
              <table className="w-full text-sm mb-8">
                ...
                <tbody>
                  ...
                  <tr className="bg-slate-50 dark:bg-gray-800 font-bold text-lg dark:text-white">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-gray-800">
                <button onClick={() => setPreviewInvoice(null)} className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-gray-300 border border-slate-300 dark:border-gray-700 rounded-md hover:bg-slate-50 dark:hover:bg-gray-800">Close</button>
