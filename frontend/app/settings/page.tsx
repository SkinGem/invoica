...
function ReadOnlyField({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div>
      <label className="block text-xs font-medium text-slate-400 dark:text-gray-500 mb-1">{label}</label>
      <div className="px-3 py-2.5 bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-lg text-sm text-slate-600 dark:text-gray-300">
        {value || '—'}
      </div>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="space-y-8 max-w-2xl">
        ...
        <div className="rounded-2xl border bg-white dark:bg-gray-900 dark:border-gray-800 p-8 flex items-center justify-center min-h-[200px]">
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      <div>
        ...
        <p className="mt-2 text-slate-500 dark:text-gray-400">Manage your account preferences and company profile.</p>
      </div>
      ...
      <div className="rounded-2xl border bg-white dark:bg-gray-900 p-6 space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold dark:text-white">Company Profile</h2>
        ...
        {!profile && (
          <div className="text-center py-8">
            <div className="w-14 h-14 bg-slate-100 dark:bg-gray-800 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <svg className="w-7 h-7 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
            </div>
            <p className="text-sm text-slate-500 dark:text-gray-400 mb-4">No company profile found. Complete the onboarding to set up your profile.</p>
            <a
              href="/"
        ...
        {profile && profile.profile_type === 'registered_company' && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-700 dark:text-gray-300">Registered Company</span>
            </div>
            ...
            {profile.verified_company_name && profile.verification_status === 'verified' && (
              <div className="p-4 rounded-lg bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/30">
                <div className="text-xs font-medium text-emerald-600 dark:text-emerald-400 mb-1">Verified Company Name</div>
                <div className="text-sm font-semibold text-emerald-900 dark:text-emerald-300">{profile.verified_company_name}</div>
                {profile.verified_at && (
                  <div className="text-xs text-emerald-500 dark:text-emerald-500 mt-1">Verified on {new Date(profile.verified_at).toLocaleDateString()}</div>
                )}
              </div>
            )}
        ...
        {profile && profile.profile_type === 'web3_project' && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-700 dark:text-gray-300">Web3 Project</span>
            </div>

            <ReadOnlyField label="Project Name" value={profile.project_name} />

            <div className="p-3 rounded-lg bg-slate-50 dark:bg-gray-800 text-sm text-slate-500 dark:text-gray-400">
        ...
        {verifyError && (
          <div className="p-3 rounded-lg bg-red-50 dark:bg-red-950/30 text-sm text-red-700 dark:text-red-400 flex items-center gap-2">
            <span>&#10007;</span> {verifyError}
          </div>
        )}
        {verifyMessage && (
          <div className={`p-3 rounded-lg text-sm flex items-center gap-2 ${profile?.verification_status === 'verified' ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400' : profile?.verification_status === 'failed' ? 'bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400' : 'bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400'}`}>
        ...
        {profile && (
          <div className="p-3 rounded-lg bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-xs text-slate-500 dark:text-gray-400 flex items-start gap-2">
            <svg className="w-4 h-4 text-slate-400 dark:text-gray-500 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <div className="rounded-2xl border bg-white dark:bg-gray-900 p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold dark:text-white">General</h2>
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1">Billing Email</label>
            {editingEmail ? (
              <input
                type="email"
                value={email}
                ...
                className="w-full px-3 py-2.5 border border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#635BFF] focus:border-transparent"
                autoFocus
              />
            ) : (
              <div className="px-3 py-2.5 bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-lg text-sm text-slate-600 dark:text-gray-300">
                {email || <span className="text-slate-400 dark:text-gray-500">Not set</span>}
              </div>
            )}
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1">Default Currency</label>
            {editingEmail ? (
              <select
                value={currency}
          ...
          {([
            ...
            <label key={key} className="flex items-start gap-3 cursor-pointer">
              <input
                ...
                className="mt-1 h-4 w-4 rounded border-slate-300 dark:border-gray-600 text-[#635BFF] focus:ring-[#635BFF]"
              />
              <div>
      ...
      {/* ─── Billing ─── */}
      <div className="rounded-2xl border bg-white dark:bg-gray-900 p-6 space-y-4">
        <h2 className="text-xl font-semibold dark:text-white">Billing</h2>
        <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-gray-800 rounded-lg">
          <div>
            ...
            <div className="text-sm text-slate-500 dark:text-gray-400">100 invoices/month</div>
          </div>
          <a
            href="/billing"
