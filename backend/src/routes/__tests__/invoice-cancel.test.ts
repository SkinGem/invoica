...
describe('POST /v1/invoices/:id/cancel', () => {
  ...
  test('200 cancels-pending — successfully cancels a PENDING invoice', async () => {
    ...
    const res = await request(app).post('/v1/invoices/inv-001/cancel');
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.data.status).toBe('CANCELLED');
  });

  test('400 already-settled — returns 400 if invoice is SETTLED', async () => {
    const fetchResult = { data: { id: 'inv-002', status: 'SETTLED' }, error: null };
    const app = buildApp(fetchResult);
    const res = await request(app).post('/v1/invoices/inv-002/cancel');
  ...
  test('400 already-cancelled — returns 400 if invoice is already CANCELLED', async () => {
    ...
    const app = buildApp(fetchResult);
    const res = await request(app).post('/v1/invoices/inv-003/cancel');
    expect(res.status).toBe(400);
    expect(res.body.success).toBe(false);
  ...
  test('404 not-found — returns 404 when invoice does not exist', async () => {
    ...
    const app = buildApp(fetchResult);
    const res = await request(app).post('/v1/invoices/nonexistent/cancel');
  ...
  test('500 db-error — returns 500 on update error', async () => {
    ...
    const app = buildApp(fetchResult, updateResult);
    const res = await request(app).post('/v1/invoices/inv-004/cancel');
