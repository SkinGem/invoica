...
describe('GET /v1/settlements/stats', () => {
  ...
  test('200 returns-shape — returns success:true with all required fields', async () => {
    const rows = [
      { amount: 100, updatedAt: new Date().toISOString() },
      { amount: 200, updatedAt: new Date().toISOString() },
    ];
    ...
    expect(res.body.data).toHaveProperty('avgAmount');
  ...
  test('200 has-total — total equals count of settled invoices', async () => {
    const rows = [
      { amount: 50,  updatedAt: new Date().toISOString() },
      { amount: 150, updatedAt: new Date().toISOString() },
      { amount: 75,  updatedAt: new Date().toISOString() },
    ];
    const app = buildApp({ data: rows, error: null });
    const res = await request(app).get('/v1/settlements/stats');
    expect(res.status).toBe(200);
    expect(res.body.data.total).toBe(3);
    expect(res.body.data.totalAmount).toBe(275);
  });

  test('200 has-total-amount — totalAmount is sum of all amounts', async () => {
    const rows = [
      { amount: 100, updatedAt: new Date().toISOString() },
      { amount: 400, updatedAt: new Date().toISOString() },
    ];
    const app = buildApp({ data: rows, error: null });
    const res = await request(app).get('/v1/settlements/stats');
    expect(res.status).toBe(200);
    expect(res.body.data.totalAmount).toBe(500);
    expect(res.body.data.avgAmount).toBe(250);
  });

  test('200 zero-when-empty — returns zeros for all fields when no settlements', async () => {
    ...
    expect(res.body.data.last30d.count).toBe(0);
  });
